...
import { blockchainSyncService } from '../../src/services/database/blockchain-sync-service';
import { databaseCacheService } from '../../src/services/database/database-cache-service';
import { reconciliationService } from '../../src/services/database/reconciliation-service';
    ...
    test('should perform full synchronization', async () => {
      ...
      User.find.mockReturnValue({
        limit: jest.fn().mockResolvedValue([
          {
            walletAddress: '0x123',
            connectedWallets: [{ address: '0x123', walletType: 'sui_wallet' }]
          }
        ])
      });
